...
header={
    ...
    'Referer':'http://218.240.145.213:9000/CTMDS/apps/pub/public.jsp',
...
def get_page_count():
    ...
    try:
        ...
        if r.status_code == 200:
            return int(math.ceil(float(r.json().get('totalRows'))/len(r.json().get('data'))))
...
def write_to_file_info():
    for i in range(1,get_page_count()+1):
        ...
        c = 0
        with open('clinic_trial_info_'+time1+'.txt','a',encoding='utf-8') as file1:
            c=0
            array = list()
            ...
            for result_info in results_info:
                ...
                result_info.pop('idd')
                json_ = json.dumps(result_info,ensure_ascii=False)
                file1.write(json_ + '\n')
                c = c + 1
            print(c)
        time.sleep(1)
...
def write_to_file():
    for i in range(1,get_page_count()+1):
        ...
        results = parse_page(json_)
        time1 = time.strftime('%Y-%m-%d',time.localtime()).replace('-','')
        with open('clinic_trial_list_'+time1+'.txt','a',encoding='utf-8') as file1:
            c=0
            for result in results:
